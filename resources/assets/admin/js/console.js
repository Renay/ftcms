$(document).ready(function() {
    ...
    isConnected($('#server').val(), $('#server').data('name'));

    $('.changeServer').on('click', function(e) {
        e.preventDefault();

        if ($(this).data('name') !== $('#server').data('name')) {
            changeServer($(this).data('id'), $(this).data('name'));
        }
    });

    $("#txtCommand").bind("enterKey",function(e){
        sendCommand($("#txtCommand").val(), $('#server').val());
    });
    ...
    $("#btnClearLog").click(function() {
        $("#groupConsole").empty();
        logInfo("Консоль очищена.");
        setTimeout(function() { $("#groupConsole").empty() }, 1500);
    });
...
function isConnected(server, name) {
    logSuccess("Соединяемся с сервером <b>" + name  + " </b>");
    $.post("/admin/console/" + server, {})
        .done(function(){
            logSuccess("Соединение с сервером установлено");

            $('#txtCommand').removeAttr('disabled');
            $('#btnSend').removeAttr('disabled');
            $('#btnClearLog').removeAttr('disabled');
            $('#chkAutoScroll').removeAttr('disabled');
        })
        .fail(function() {
            logDanger("Подключится к данному серверу не удалось");

            $('#txtCommand').attr('disabled', '');
            $('#btnSend').attr('disabled', '');
            $('#btnClearLog').attr('disabled', '');
            $('#chkAutoScroll').attr('disabled', '');
        });
}

function changeServer(id, name) {
    $('#server').val(id).data('name', name);
    return isConnected(id, name);
}
...
function sendCommand(command, server) {
    if (command == "") {
        return swal({
            type: 'warning',
            title: 'Ты чо?',
            text: 'Решил отправить пустую команду? Это ж не дело...',
            showConfirmButton: false,
            timer: 1500
        });
    }
    logMsg(command, ">_", "success");

    $.post("/admin/console/sendcmd/" + server, { cmd: command }, function(json) {
            if(json.status) {
                if(json.status == 'success' && json.response && json.command){
                    ...
                    logDanger("Не верный формат ответа RCON api");
                }
            }
            else{
                logDanger("Нет соединения с сервером");
            }
        }, 'json')
        .fail(function() {
            logDanger("RCON api отдал ошибку");
        });
}
